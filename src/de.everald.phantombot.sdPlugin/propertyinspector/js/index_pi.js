...
$SD.on('receivedGlobalSettings', (returnValue) => {
    console.log('receivedGlobalSettings', returnValue);
    if (window.xtWindow && !window.xtWindow.closed) {
        w.postMessage(
            Object.assign({}, returnValue, {action: $SD.actionInfo.action})
            ,'*');
    }
});
 ...
 */
 ...
 function saveSettings(sdpi_collection) {
    console.log('sdpi_collection: ', sdpi_collection);
...
function gotCallbackFromWindow(parameter) {
    ...
    sendValueToPlugin(parameter, 'settings');
}
